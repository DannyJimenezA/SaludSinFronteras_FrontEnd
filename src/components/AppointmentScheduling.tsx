...
interface AppointmentSchedulingProps {
  // ✅ ahora puede recibir params desde la navegación
  onNavigate: (screen: string, params?: { doctorId?: number }) => void;
  preselectedDoctorId?: number;
  preselectedDateISO?: string; // opcional
}
...
export function AppointmentScheduling({
  ...
  const { data: slots, isLoading: slotsLoading, isRefetching } = useDoctorAvailability(
    typeof doctorId === "number" ? doctorId : undefined,
    dateISO || undefined
  );
  ...
  useEffect(() => {
    // si venimos con doctor precargado, no hacemos nada extra;
    // el propio hook pedirá la disponibilidad
  }, [preselectedDoctorId]);

  /* ------------------ Acciones ------------------ */
  async function handleBook(slotId: number | string) {
    if (!canQuery) return;

    try {
      await createAppt.mutateAsync({
        doctorId: doctorId as number,
        slotId,                 // <-- tu API usa SlotId
        modality: apptType === "videollamada" ? "video" : "inperson",
      });
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                {/* TEMPORAL: campo Doctor ID hasta que tengas /doctors (search/list) */}
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={apptType}
                    onValueChange={(v: ApptType) => setApptType(v)}
                  >
                    <SelectTrigger>
                  ...
                  <Card key={s.id} className="hover:shadow-sm">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{timeLabel(s.startAt)}</span>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
